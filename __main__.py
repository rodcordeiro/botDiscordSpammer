...
def main(argv):
  opts, args = getopt.getopt(argv, "ht:x:m:r:g:c:p:s:", [
    "two-factor=", "xp-boster=", "market=", "remove-market=", "get-mega=",
    "candies=", "pokemon-select=", "spamming="
  ])
  has2f = False
  twofactorcode = ''
  actions = {}
  for opt, arg in opts:
    if opt == '-h':
      logger.info(
        'Bot spammer for Poketwo! Look below for examples of the commands. For better understanding, read the code! (sorry, theres no doc for this masterpiece ://) '
      )
